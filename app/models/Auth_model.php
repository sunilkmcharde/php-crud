...
class Auth_model extends CI_Model
{
    ...
    public $new_password;
    public $identity;
    public $_crud_where = array();
    public $_crud_select = array();
    public $_crud_like = array();
    public $_crud_limit = NULL;
    public $_crud_offset = NULL;
    public $_crud_order_by = NULL;
    public $_crud_order = NULL;
    protected $_crud_hooks;
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $this->db->insert('crud_user_logins', $ldata);
    ...
    public function where($where, $value = NULL)
    {
        ...
        array_push($this->_crud_where, $where);
    ...
    public function like($like, $value = NULL, $position = 'both')
    {
        ...
        array_push($this->_crud_like, $like);
    ...
    public function order_by($by, $order = 'desc')
    {
        ...
        $this->_crud_order_by = $by;
        $this->_crud_order = $order;
    ...
    public function users($groups = NULL)
    {
        ...
        if (isset($this->_crud_select) && !empty($this->_crud_select)) {
            foreach ($this->_crud_select as $select) {
                $this->db->select($select);
            }

            $this->_crud_select = array();
        } else {
            //default selects
        ...
        if (isset($this->_crud_where) && !empty($this->_crud_where)) {
            foreach ($this->_crud_where as $where) {
                $this->db->where($where);
            }

            $this->_crud_where = array();
        }

        if (isset($this->_crud_like) && !empty($this->_crud_like)) {
            foreach ($this->_crud_like as $like) {
                $this->db->or_like($like);
            }

            $this->_crud_like = array();
        }

        if (isset($this->_crud_limit) && isset($this->_crud_offset)) {
            $this->db->limit($this->_crud_limit, $this->_crud_offset);

            $this->_crud_limit = NULL;
            $this->_crud_offset = NULL;
        } else if (isset($this->_crud_limit)) {
            $this->db->limit($this->_crud_limit);

            $this->_crud_limit = NULL;
        }

        //set the order
        if (isset($this->_crud_order_by) && isset($this->_crud_order)) {
            $this->db->order_by($this->_crud_order_by, $this->_crud_order);

            $this->_crud_order = NULL;
            $this->_crud_order_by = NULL;
        }
    ...
    {
        $this->_crud_hooks->{$event}[$name] = new stdClass;
        $this->_crud_hooks->{$event}[$name]->class = $class;
        $this->_crud_hooks->{$event}[$name]->method = $method;
        $this->_crud_hooks->{$event}[$name]->arguments = $arguments;
    }

    public function remove_hook($event, $name)
    {
        if (isset($this->_crud_hooks->{$event}[$name])) {
            unset($this->_crud_hooks->{$event}[$name]);
        }
    }

    public function remove_hooks($event)
    {
        if (isset($this->_crud_hooks->$event)) {
            unset($this->_crud_hooks->$event);
        }
    }

    protected function _call_hook($event, $name)
    {
        if (isset($this->_crud_hooks->{$event}[$name]) && method_exists($this->_crud_hooks->{$event}[$name]->class, $this->_crud_hooks->{$event}[$name]->method)) {
            $hook = $this->_crud_hooks->{$event}[$name];
    ...
    public function trigger_events($events)
    {
        if (is_array($events) && !empty($events)) {
            ...
            if (isset($this->_crud_hooks->$events) && !empty($this->_crud_hooks->$events)) {
                foreach ($this->_crud_hooks->$events as $name => $hook) {
