...
class Auth extends MY_Controller
{
    ...
    function login($m = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $login_responce = $this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember);
            if ($login_responce) {
                $this->session->set_flashdata('message', $this->ion_auth->messages());
                $referrer = $this->session->userdata('requested_page') ? $this->session->userdata('requested_page') : 'admin/dashboard';
                redirect($referrer);
            } else {
